...
public class MyBufferedTripsTest {
    ...
    @Test
    void FrenchMandarineTestGaming74_CE() {
        BufferedTrips trips = new BufferedTrips(stringStationList, bytesBuffer);
        assertEquals(95, trips.size());
        assertEquals(1784, trips.routeId(3));
        assertEquals(stringStationList.get(297), trips.destination(3));
        assertEquals(12874, trips.routeId(43));
        assertEquals(stringStationList.get(300), trips.destination(43));
    }
    ...
    private static final ByteBuffer CONNECTIONS_BUFFER = byteBuffer(
            "00 64 01 F4 00 C8 02 26 00 30 39 03 " +
                    "00 65 02 58 00 C9 02 62 00 D4 31 07 " +
                    "00 66 02 BC 00 CA 02 EE 00 03 E7 0F"
    );


    private static final ByteBuffer SUCC_BUFFER = byteBuffer(
    ...
    @Test
    void testSizeWorksSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        assertEquals(3, bc.size(), "Expected 3 connection records");
    }


    @Test
    void testDepStopIdWorksSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        // Record 0: 100, Record 1: 101, Record 2: 102.
        assertEquals(100, bc.depStopId(0));
        assertEquals(101, bc.depStopId(1));
        assertEquals(102, bc.depStopId(2));
    }


    @Test
    void testDepMinsWorksSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        // Record 0: 500, Record 1: 600, Record 2: 700.
        assertEquals(500, bc.depMins(0));
        assertEquals(600, bc.depMins(1));
        assertEquals(700, bc.depMins(2));
    }


    @Test
    void testArrStopIdWorksSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        // Record 0: 200, Record 1: 201, Record 2: 202.
        assertEquals(200, bc.arrStopId(0));
        assertEquals(201, bc.arrStopId(1));
        assertEquals(202, bc.arrStopId(2));
    }


    @Test
    void testArrMinsWorksSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        // Record 0: 550, Record 1: 610, Record 2: 750.
        assertEquals(550, bc.arrMins(0));
        assertEquals(610, bc.arrMins(1));
        assertEquals(750, bc.arrMins(2));
    }


    @Test
    void testTripIdWorksSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        // For Record 0: TRIP_POS_ID = 0x00303903 -> tripId = 12345 (0x3039)
        assertEquals(12345, bc.tripId(0));
        // For Record 1: TRIP_POS_ID = 0x00D43107 -> tripId = 54321 (0xD431)
        assertEquals(54321, bc.tripId(1));
        // For Record 2: TRIP_POS_ID = 0x0003E70F -> tripId = 999 (0x03E7)
        assertEquals(999, bc.tripId(2));
    }


    @Test
    void testTripPosWorksSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        // For Record 0: TRIP_POS_ID = 0x00303903 -> tripPos = 3
        assertEquals(3, bc.tripPos(0));
        // For Record 1: TRIP_POS_ID = 0x00D43107 -> tripPos = 7
        assertEquals(7, bc.tripPos(1));
        // For Record 2: TRIP_POS_ID = 0x0003E70F -> tripPos = 15 (0x0F)
        assertEquals(15, bc.tripPos(2));
    }


    @Test
    void testNextConnectionIdWorksSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        // From our succBuffer:
        // Record 0 -> 1, Record 1 -> 2, Record 2 -> 0.
        assertEquals(1, bc.nextConnectionId(0));
        assertEquals(2, bc.nextConnectionId(1));
        assertEquals(0, bc.nextConnectionId(2));
    }


    @Test
    void testInvalidIndexThrowsSTEF() {
        BufferedConnections bc = new BufferedConnections(CONNECTIONS_BUFFER, SUCC_BUFFER);
        // Ensure that invalid indices throw IndexOutOfBoundsException.
        assertAll(
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.depStopId(-1)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.depStopId(3)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.depMins(-1)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.depMins(3)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.arrStopId(-1)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.arrStopId(3)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.arrMins(-1)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.arrMins(3)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.tripId(-1)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.tripId(3)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.tripPos(-1)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.tripPos(3)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.nextConnectionId(-1)),
                () -> assertThrows(IndexOutOfBoundsException.class, () -> bc.nextConnectionId(3))
        );
    }


    @Test
    void testEmptyBuffersSTEF() {
        ByteBuffer empty = ByteBuffer.allocate(0).asReadOnlyBuffer();
        // Construct a succBuffer for an empty connections buffer.
        ByteBuffer emptySucc = ByteBuffer.allocate(0).asReadOnlyBuffer();
        BufferedConnections bc = new BufferedConnections(empty, emptySucc);
        assertEquals(0, bc.size(), "Empty buffer should report size 0");
    ...
    @Test
    void testInvalidBufferLengthSTEF() {
        // Create a connections buffer whose length is not a multiple of 12 (record size).
        ByteBuffer invalidBuffer = ByteBuffer.allocate(10).asReadOnlyBuffer();
        // The succBuffer can be arbitrary here.
        assertThrows(IllegalArgumentException.class,
                () -> new BufferedConnections(invalidBuffer, SUCC_BUFFER),
                "Buffer with invalid length should throw IllegalArgumentException");
    }
